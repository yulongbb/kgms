        ...
        with open('data.csv', 'w', encoding='utf-8') as c:
            ...
            for category in response.css('.product'):
                ...
                for product in category.css('li>a'):
                    ...
                    writer.writerow([product.css('strong::text').get(
                    ), category.css('.caption::text').get(), "类型"])
                    writer.writerow([product.css('strong::text').get(), 'https://www.starbucks.com.cn' + product.css(
                        '.preview').xpath('@style').re(r'background-image: url(\s)*(.*)')[1].split('"')[1], "图片"])
